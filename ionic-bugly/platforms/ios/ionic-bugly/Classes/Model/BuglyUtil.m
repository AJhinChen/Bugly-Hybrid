...
+ (instancetype)shareInstance {
    static BuglyUtil *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[BuglyUtil alloc] init];
    });
    return instance;
}
...
+ (NSString *)buglyObjectId {
    ...
    NSNumber *count = [[BuglyUtil shareInstance] getNumberById:BuglyItemCountTag fromTable:BuglyCountTableName];
...
+ (NSString *)categoriesCountId {
    NSNumber *count = [[BuglyUtil shareInstance] getNumberById:BuglyCategoriesCountTag fromTable:BuglyCountTableName];
    NSString *categoriesCountId = [NSString stringWithFormat:@"%d",count.intValue + 1];
    return categoriesCountId;
}

+ (NSString *)productsCountId {
    NSNumber *count = [[BuglyUtil shareInstance] getNumberById:BuglyProductsCountTag fromTable:BuglyCountTableName];
    NSString *categoriesCountId = [NSString stringWithFormat:@"%d",count.intValue + 1];
    return categoriesCountId;
}

+ (NSString *)stateCountId {
    NSNumber *count = [[BuglyUtil shareInstance] getNumberById:BuglyStateCountTag fromTable:BuglyCountTableName];
    NSString *categoriesCountId = [NSString stringWithFormat:@"%d",count.intValue + 1];
    return categoriesCountId;
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        BuglyUtil *bugUtil = [[BuglyUtil alloc] initDBWithName:BuglyDBName];
        // 创建表，如果已存在，则忽略该操作
...
- (void)insertBugItem:(NSDictionary *)item {
    NSString *buglyObjectId = [BuglyUtil buglyObjectId];
...
- (void)insertCategory:(NSDictionary *)category {
    NSString *categoriesCountId = [BuglyUtil categoriesCountId];
...
- (void)insertProduct:(NSDictionary *)product {
    NSString *productsCountId = [BuglyUtil productsCountId];
...
- (void)insertState:(NSDictionary *)state {
    NSString *stateCountId = [BuglyUtil stateCountId];
...
- (NSArray *)getAllItemsContainsString:(NSString *)string {
    if ([BuglyUtil checkTableName:BuglyItemsTableName] == NO) {
        return nil;
    }
...
- (NSNumber *)getCountByTableName:(NSString *)tableName {
    if ([BuglyUtil checkTableName:tableName] == NO) {
        return nil;
    }
