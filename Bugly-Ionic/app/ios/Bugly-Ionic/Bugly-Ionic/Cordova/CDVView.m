...
- (void)__init
{
    if ((self != nil) && !self.initialized) {
        _commandQueue = [[CDVViewCommandQueue alloc] initWithView:self];
        _commandDelegate = [[CDVViewCommandDelegateImpl alloc] initWithView:self];
...
- (void)createGapView
{
    CGRect webViewBounds = self.bounds;
    
    webViewBounds.origin = self.bounds.origin;
